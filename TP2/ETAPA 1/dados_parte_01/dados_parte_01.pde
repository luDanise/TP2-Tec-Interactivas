...
float anguloCameraX, anguloCameraY = 0;
...
void mouseDragged() {
   beginCamera();
   camera();
   float dy = (mouseY - pmouseY) * velocidadRotacion;
   float dx = (mouseX - pmouseX) * velocidadRotacion;
   
   anguloCameraY += dy;
   anguloCameraX += dx;
    
   rotateY(radians(anguloCameraX));
   rotateX(radians(anguloCameraY));
